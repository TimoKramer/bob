...
public class APIServer extends AbstractVerticle {
    ...
    public APIServer(String apiSpec, String host, int port, RabbitMQClient queue) {
    ...
    public void start(Promise<Void> startPromise) {
        this.queue
            ...
            .compose(router -> serverFrom(this.vertx, router, this.host, this.port, this.queue))
    ...
    private static Future<HttpServer> serverFrom(Vertx vertx, OpenAPI3RouterFactory routerFactory, String host, int port, RabbitMQClient queue) {
        final var router = routerFactory
            .addHandlerByOperationId("HealthCheck", ctx -> Handlers.healthCheckHandler(ctx, queue))
            ...
            .addHandlerByOperationId("PipelineLogs", ctx -> Handlers.pipelineLogsHandler(ctx))
            .addHandlerByOperationId("PipelineStatus", ctx -> Handlers.pipelineStatusHandler(ctx))
            .addHandlerByOperationId("PipelineArtifactFetch", ctx -> Handlers.pipelineArtifactHandler(ctx, queue))
            .addHandlerByOperationId("PipelineList", ctx -> Handlers.pipelineListHandler(ctx))
            ...
            .addHandlerByOperationId("ResourceProviderList", ctx -> Handlers.resourceProviderListHandler(ctx))
            ...
            .addHandlerByOperationId("ArtifactStoreList", ctx -> Handlers.artifactStoreListHandler(ctx))
